...
The contracted forms of the Gauss-Codazzi equations are also provided. These are:
\begin{center}
...
The right-hand side of the contracted Gauss-Codazzi equations is provided as an object. In some case it is useful to
construct modified versions of the G-C equations using $T_{\mu \nu}$ or a specific decomposition via \text\tt{grdef}. \\
...
%     \texttt{GCeqn1RHS}        &  R_{a b c d} + \epsilon \left( K_{ab} K_{bc} - K_{ac} K_{bd} \right) $  \\
      \texttt{HCGeqn}        & $-2 \epsilon G_{\mu \nu} n^\mu n^\nu = R + \epsilon \left( K^{i j}K_{ij} -K^2 \right)$  \\
      \texttt{HCGeqnRHS}        & $R + \epsilon \left( K^{i j}K_{ij} -K^2 \right)$  \\
      \texttt{PCGeqn}        & $G_{\mu \nu} e^\mu_a n^\nu = K^b_{a|b} - K_{,a}$ \\
      \texttt{PCGeqnRHS}        & $ K^b_{a|b} - K_{,a}$ \\
    \end{tabular}
