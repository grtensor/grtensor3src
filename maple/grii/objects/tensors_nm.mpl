...
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
 1/2*grG_gdndnpdn_[grG_metricName,a1_,a3_,a2_] + 1/2*grG_gdndnpdn_[grG_metricName,a2_,a3_,a1_] -
 1/2*grG_gdndnpdn_[grG_metricName,a1_,a2_,a3_]:
end:
...
grG_ObjDef[Chr(dn,dn,up)][grC_calcFnParms] := # raise last index
  'grG_gupup_[grG_metricName,a3_,s1_]' * 'grG_Chrdndndn_[grG_metricName, a1_,a2_,s1_]':
...
global grG_metricName;
 s := grG_Rdndndndn_[grG_metricName,a1_,a2_,a3_,a4_]
     -1/(Ndim||grG_metricName-2)*
		 (grG_gdndn_[grG_metricName,a1_,a3_]*grG_Rdndn_[grG_metricName,a4_,a2_] -
		  grG_gdndn_[grG_metricName,a1_,a4_]*grG_Rdndn_[grG_metricName,a3_,a2_] -
		  grG_gdndn_[grG_metricName,a2_,a3_]*grG_Rdndn_[grG_metricName,a4_,a1_] +
		  grG_gdndn_[grG_metricName,a2_,a4_]*grG_Rdndn_[grG_metricName,a3_,a1_]) +
	    1/(Ndim||grG_metricName - 1)/(Ndim||grG_metricName - 2)*
	    grG_scalarR_[grG_metricName]*
	    (grG_gdndn_[grG_metricName,a1_,a3_]*grG_gdndn_[grG_metricName,a4_,a2_] -
	     grG_gdndn_[grG_metricName,a1_,a4_]*grG_gdndn_[grG_metricName,a3_,a2_] ):
 RETURN(s):
end:
...
grG_ObjDef[C(dn,dn,up,up)][grC_calcFnParms] := 'grG_gupup_[grG_metricName,s1_,a3_]' * 'grG_gupup_[grG_metricName,s2_,a4_]' *
  'grG_Cdndndndn_[grG_metricName,a1_,a2_,s1_,s2_]':
...
global grG_metricName;
  ...
  for i to Ndim||grG_metricName do
     s := s + grG_gdndn_[grG_metricName,i,i] * (` d` * grG_xup_[grG_metricName,i]^`2 `);
     for j from i+1 to Ndim||grG_metricName  do
     #
     # need a `d ` and ` d` to avoid e.g. d^2 u v
     #
	 s := s + 2 * grG_gdndn_[grG_metricName,i,j] * ( ` d`*(grG_xup_[grG_metricName,i])^` `)
		    * (`d `*(grG_xup_[grG_metricName,j])^` `);
     od:
  od:
  RETURN(s):
end:
...
global grG_metricName;
	if grF_checkIfAssigned ( e(bdn,dn) ) then
		...
		for i to Ndim[grG_metricName] do
		  ...
		  if i <> j then
		     grG_gdndn_[grG_metricName,j,i] := grG_gdndn_[grG_metricName,i,j]:
	          fi:
			grG_gdndn_[grG_metricName,i,j] := 0:
			for a to Ndim[grG_metricName] do
				grG_gdndn_[grG_metricName,i,j] := grG_gdndn_[grG_metricName,i,j]
				    + grG_ebdndn_[grG_metricName,a,i]*grG_ebupdn_[grG_metricName,a,j]:
				od:
			od:
		od:
...
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
global  grG_gupup_, grG_metricName;
local	i, j, a:
	if grF_checkIfAssigned ( e(bdn,up) ) then
		...
		for i to Ndim[grG_metricName] do
		  for j from i to Ndim[grG_metricName] do
		  if i <> j then
		     grG_gupup_[grG_metricName,j,i] := grG_gupup_[grG_metricName,i,j]:
	          fi:
				grG_gupup_[grG_metricName,i,j] := 0:
				for a to Ndim[grG_metricName] do
					grG_gupup_[grG_metricName,i,j] := grG_gupup_[grG_metricName,i,j] + grG_ebdnup_[grG_metricName,a,i]*grG_ebupup_[grG_metricName,a,j]:
				od:
			od:
		od:
...
grG_ObjDef[g(dn,dn,pdn)][grC_calcFn] := grF_calc_sum0:
grG_ObjDef[g(dn,dn,pdn)][grC_calcFnParms] :=
'diff(grG_gdndn_[grG_metricName, a1_, a2_], grG_xup_[grG_metricName, a3_ ])':
...
grG_ObjDef[g(dn,dn,pdn,pdn)][grC_calcFn] := grF_calc_sum0:
grG_ObjDef[g(dn,dn,pdn,pdn)][grC_calcFnParms] :=
    'diff(grG_gdndnpdn_[grG_metricName, a1_,a2_,a3_ ], grG_xup_[grG_metricName, a4_ ])':
...
grG_ObjDef[G(dn,dn)][grC_calcFnParms] := grG_Rdndn_[grG_metricName,a1_,a2_]
	- 1/2*(grG_gdndn_[grG_metricName,a1_,a2_] * grG_scalarR_[grG_metricName]):
grG_ObjDef[G(dn,dn)][grC_symmetry] := grF_sym_sym2:
...
global grG_metricName;
 ...
 for a to Ndim||grG_metricName do
   if not a2_ = a then
      s := s + diff(grG_Chrdndnup_[grG_metricName,a1_,a2_,a],grG_xup_[grG_metricName,a])
	     - diff(grG_Chrdndnup_[grG_metricName,a,a1_,a],grG_xup_[grG_metricName,a2_]):
   fi:
   for b to Ndim||grG_metricName do
      s := s + grG_Chrdndnup_[grG_metricName,a1_,a2_,b] * grG_Chrdndnup_[grG_metricName,a,b,a]
	     - grG_Chrdndnup_[grG_metricName,a1_,a,b] * grG_Chrdndnup_[grG_metricName,b,a2_,a]:
   od:
 od:
end:
...
global grG_metricName;
 ...
 if (a1_ = a3_ and a2_ = a4_) then
    s := diff( grG_gdndnpdn_[grG_metricName,a1_,a2_,a2_], grG_xup_[grG_metricName,a1_]):
 else
    s := 1/2*(diff(grG_gdndnpdn_[grG_metricName,a1_,a4_,a2_] , grG_xup_[grG_metricName,a3_])
	  +  diff(grG_gdndnpdn_[grG_metricName,a2_,a3_,a1_] , grG_xup_[grG_metricName,a4_]) ):
 fi:
 if (a1_ = a4_ and a3_ = a2_) then
     s := s - diff(grG_gdndnpdn_[grG_metricName,a1_,a3_,a3_] , grG_xup_[grG_metricName,a1_] ):
 else
     s := s - 1/2*(diff(grG_gdndnpdn_[grG_metricName,a1_,a3_,a2_], grG_xup_[grG_metricName,a4_])
	    + diff(grG_gdndnpdn_[grG_metricName,a2_,a4_,a1_], grG_xup_[grG_metricName,a3_]) ):
 fi:
 for m to Ndim||grG_metricName do
   for n to Ndim||grG_metricName do
     s := s + grG_gupup_[grG_metricName,m,n]*
	  (grG_Chrdndndn_[grG_metricName,a1_,a4_,m] * grG_Chrdndndn_[grG_metricName,a2_,a3_,n]
	  - grG_Chrdndndn_[grG_metricName,a1_,a3_,m] * grG_Chrdndndn_[grG_metricName,a2_,a4_,n])
   od:
 od:
 s;
end:
...
# raise indices
grG_ObjDef[R(up,up,dn,dn)][grC_calcFnParms] := 'grG_gupup_[grG_metricName,s1_,a1_]' * 'grG_gupup_[grG_metricName,s2_,a2_]' *
  'grG_Rdndndndn_[grG_metricName,s1_,s2_,a3_,a4_]':
...
global grG_metricName;
  s := 0;
  for a to Ndim||grG_metricName do
    s := s + grG_gupup_[grG_metricName, a, a] * grG_Rdndn_[grG_metricName, a, a]:
    for b from a+1 to Ndim||grG_metricName do
       s := s + 2 * grG_gupup_[grG_metricName, a, b] * grG_Rdndn_[grG_metricName, a, b]:
    od:
  od:
  RETURN( s):
end:
...
global grG_metricName;
   s := 0:
   for i to Ndim||grG_metricName do
      s := s + grG_Rupdn_[grG_metricName,i,i] * grG_Rupdn_[grG_metricName,i,i]:
      for j from i+1 to Ndim||grG_metricName do
	   s := s + 2 * grG_Rupdn_[grG_metricName,i,j] * grG_Rupdn_[grG_metricName,j,i]:
      od;
   od;
   RETURN(s):
end:
...
# explicit formula for N=4 & calc
 s :=
4*(grG_||root1[grG_metricName,1,2,1,2]*grG_||root2[grG_metricName,1,2,1,2]+
grG_||root1[grG_metricName,1,2,1,3]*grG_||root2[grG_metricName,1,3,1,2] +
grG_||root1[grG_metricName,1,2,1,4]*grG_||root2[grG_metricName,1,4,1,2] +
grG_||root1[grG_metricName,1,2,2,3]*grG_||root2[grG_metricName,2,3,1,2] +
grG_||root1[grG_metricName,1,2,2,4]*grG_||root2[grG_metricName,2,4,1,2] +
grG_||root1[grG_metricName,1,2,3,4]*grG_||root2[grG_metricName,3,4,1,2] +
grG_||root1[grG_metricName,1,3,1,2]*grG_||root2[grG_metricName,1,2,1,3] +
grG_||root1[grG_metricName,1,3,1,3]*grG_||root2[grG_metricName,1,3,1,3] +
grG_||root1[grG_metricName,1,3,1,4]*grG_||root2[grG_metricName,1,4,1,3] +
grG_||root1[grG_metricName,2,4,1,2]*grG_||root2[grG_metricName,1,2,2,4] +
grG_||root1[grG_metricName,2,4,1,3]*grG_||root2[grG_metricName,1,3,2,4] +
grG_||root1[grG_metricName,2,4,1,4]*grG_||root2[grG_metricName,1,4,2,4] +
grG_||root1[grG_metricName,2,4,2,3]*grG_||root2[grG_metricName,2,3,2,4] +
grG_||root1[grG_metricName,2,4,2,4]*grG_||root2[grG_metricName,2,4,2,4] +
grG_||root1[grG_metricName,1,3,2,3]*grG_||root2[grG_metricName,2,3,1,3] +
grG_||root1[grG_metricName,1,3,2,4]*grG_||root2[grG_metricName,2,4,1,3] +
grG_||root1[grG_metricName,1,3,3,4]*grG_||root2[grG_metricName,3,4,1,3] +
grG_||root1[grG_metricName,1,4,1,2]*grG_||root2[grG_metricName,1,2,1,4] +
grG_||root1[grG_metricName,1,4,1,3]*grG_||root2[grG_metricName,1,3,1,4] +
grG_||root1[grG_metricName,1,4,1,4]*grG_||root2[grG_metricName,1,4,1,4] +
grG_||root1[grG_metricName,1,4,2,3]*grG_||root2[grG_metricName,2,3,1,4] +
grG_||root1[grG_metricName,1,4,2,4]*grG_||root2[grG_metricName,2,4,1,4] +
grG_||root1[grG_metricName,1,4,3,4]*grG_||root2[grG_metricName,3,4,1,4] +
grG_||root1[grG_metricName,2,3,1,2]*grG_||root2[grG_metricName,1,2,2,3] +
grG_||root1[grG_metricName,2,3,1,3]*grG_||root2[grG_metricName,1,3,2,3] +
grG_||root1[grG_metricName,2,3,1,4]*grG_||root2[grG_metricName,1,4,2,3] +
grG_||root1[grG_metricName,2,3,2,3]*grG_||root2[grG_metricName,2,3,2,3] +
grG_||root1[grG_metricName,2,3,2,4]*grG_||root2[grG_metricName,2,4,2,3] +
grG_||root1[grG_metricName,2,3,3,4]*grG_||root2[grG_metricName,3,4,2,3] +
grG_||root1[grG_metricName,2,4,3,4]*grG_||root2[grG_metricName,3,4,2,4] +
grG_||root1[grG_metricName,3,4,1,2]*grG_||root2[grG_metricName,1,2,3,4] +
grG_||root1[grG_metricName,3,4,1,3]*grG_||root2[grG_metricName,1,3,3,4] +
grG_||root1[grG_metricName,3,4,1,4]*grG_||root2[grG_metricName,1,4,3,4] +
grG_||root1[grG_metricName,3,4,2,3]*grG_||root2[grG_metricName,2,3,3,4] +
grG_||root1[grG_metricName,3,4,2,4]*grG_||root2[grG_metricName,2,4,3,4] +
grG_||root1[grG_metricName,3,4,3,4]*grG_||root2[grG_metricName,3,4,3,4]):
	...
	for a to Ndim||grG_metricName - 1 do
	  for b from a+1 to Ndim||grG_metricName do
		...
		s := s + 4 * grG_||root1[grG_metricName,a,b,c,d]* grG_||root2[grG_metricName,c,d,a,b];
	      od:
	    od:
...
grG_ObjDef[S(dn,dn)][grC_calcFnParms] :=
	'grG_Rdndn_[grG_metricName,a1_,a2_]' -
	(1/Ndim[grG_metricName])*'grG_gdndn_[grG_metricName,a1_,a2_] * grG_scalarR_[grG_metricName]':
