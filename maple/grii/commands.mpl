...
grcalcd := proc() grcalc(args); grdisplay(args); end:
...
#------------------------------------------------------------------------------
grF_calcObj := proc ( )
global  grG_fnCode, grG_simp, grG_calc, grG_callComp:
local	i:
...
local object, objectName, iList, coordNum_seq,  a, b, operands,
  ...
  if not assigned(
    grG_||(grG_ObjDef[objectName][grC_root])[operands, coordNum_seq]) then
      ERROR(`The requested component has not been calculated.`):
  else
    grG_||(grG_ObjDef[objectName][grC_root])[operands, coordNum_seq];
  fi:
...
grmap :=  proc()
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
global  grG_preSeq, grG_postSeq, grG_simpHow, grG_calc, grG_simp, grG_callComp, grG_fnCode;
...
local 	a, b, G, req, metrictype, bupflag, bdnflag, gname, metricdir, savename:
	...
	if assigned ( grG_Info_[gname] )  then
        ...
        G||coord := [grG_xup_[gname,1]]:
	for a from 2 to G||gdim do
		G||coord := [op(G||coord), grG_xup_[gname,a]]:
	od:
...
eta(bup,bup) ) then
	...
	if metrictype = grG_g then
		...
		for a to G||gdim do
			for b to G||gdim do
				G||metric[a,b] := grG_gdndn_[gname,a,b]:
			od:
		od:
	else
		bdnflag := false:
