...
global	grG_metricName, grG_preSeq, grG_postSeq, grG_ebdndn_, grG_etabupbup_,
	grG_simpHow, grG_lastObjectSeq, grG_etabdnbdn_, grG_calc, grG_fnCode,
	grG_NPldn_, grG_NPndn_, grG_NPmdn_, grG_NPmbardn_:
...
for a to 4 do
  for b from a to 4 do
    grG_etabupbup_[gname,b,a] := grG_etabupbup_[gname,a,b]:
    grG_etabupbup_[gname,a,b] := simplify ( ip[a,b] ):
  od:
  for b to 4 do
    grG_ebdndn_[gname,a,b] := grG_simpHow ( grG_preSeq, bv[a,b], grG_postSeq):
  od:
od:
...
for a to 4 do
  grG_NPldn_[gname,a] := grG_ebdndn_[gname,1,a]:
  grG_NPndn_[gname,a] := grG_ebdndn_[gname,2,a]:
  grG_NPmdn_[gname,a] := grG_ebdndn_[gname,3,a]:
  grG_NPmbardn_[gname,a] := grG_ebdndn_[gname,4,a]:
od:
...
if not assigned ( grG_sig_[gname] ) then
  printf ( `The metric signature has not been assigned.\n` ):
  sig := grF_readstat ( `Please enter the metric signature as an integer: `, [], `nptetrad` ):
...
global	grG_sig_, grG_gdndn_, grG_ds_, grG_gupup_, grG_calc, grG_fnCode:
...
for a to ndim do
  for b from a to ndim do
    grG_gdndn_[gname,a,b] := -grG_gdndn_[gname,a,b]:
  od:
od:
...
if grF_checkIfAssigned ( g(up,up) ) then
  for a to ndim do
    for b from a to ndim do
      grG_gupup_[gname,a,b] := -grG_gupup_[gname,a,b]:
    od:
  od:
fi:
...
if lnspace <> [] then  
  ...
  for a to 2 do
    ...
    for b to 4 while not found do
      if lnspace[a] = grG_xup_[gname,b] then
        idxnbr := idxnbr,b:
        found := true:
      fi:
    od:
...
nullCoords := {}:
for a to 4 do
  if member ( grG_xup_[grG_metricName,a], cset ) then
      complexCoords := true:
  elif obv[a,a] = 0 then
...
local a, i, coordList, complexCoords, idxseq:
  coordList := [ seq ( grG_xup_[grG_metricName,i], i=1..ndim ) ]:
...
for a to 4 do
  grG_ebdnup_[gname, v2, a] := grG_ebdnup_[gname, v2, a] +
    Ac*grG_ebdnup_[gname,3,a] + A*grG_ebdnup_[gname,4,a] +
    A*Ac*grG_ebdnup_[gname, v2, a]:

  grG_ebdnup_[gname, 3, a] := grG_ebdnup_[gname, 3, a] +
    A*grG_ebdnup_[gname, v1, a]:

  grG_ebdnup_[gname, 4, a] := grG_ebdnup_[gname, 4, a] +
    Ac*grG_ebdnup_[gname, v1, a]:
od:
...
for a to 4 do
  grG_ebdnup_[gname, 1, a] := grG_ebdnup_[gname, 1, a]/A:

  grG_ebdnup_[gname, 2, a] := grG_ebdnup_[gname, 2, a]*A:

  grG_ebdnup_[gname, 3, a] := grG_ebdnup_[gname, 3, a]*exp( I*theta):

  grG_ebdnup_[gname, 4, a] := grG_ebdnup_[gname, 4, a]*exp(-I*theta):
od:
