...
global  Ndim, Ndim_, constraint_, grG_Ndim, grG_xup_, grG_constraint,
  grG_metricName, Info_, grG_Info_, sig_, grG_sig_, grG_complexSet_, 
  grG_metricSet;
local  a, b, i, j, ndim, gtype, ip, npip, g, bu, bd, bup, bdn, grinit, stsig:
...
# set dimension.
#
  if assigned ( Ndim ) then
    Ndim||gname := Ndim:
  elif assigned ( Ndim_ ) then
    Ndim||gname := Ndim_:
  else
    ERROR ( `Invalid metric file. grload could not find Ndim or Ndim_.` ):
  fi:
...
#
  if assigned ( Ndim ) then
    for a to ndim do
      grG_xup_[gname,a] := x||a:
      x||a := evaln ( x||a ):
    od:
  else
    for a to ndim do
      grG_xup_[gname,a] := x||a||_:
      x||a||_ := evaln ( x||a||_ ):
    od:
  fi:
...
global  grG_coordStr, grG_defaultMetric, grG_metricSet,
  grG_default_metricName, grG_metricName, grG_constant_eta,
  complexSet_, Ndim:
local i, gname, setMetric;
  ...
  for i to Ndim||gname do
      grG_coordStr[gname,i] := convert(grG_xup_[gname,i], name);
  od:
...
qload := proc( metric)
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
global  grOptionFileCharacter, grOptionMetricPath:
local mfile, m;
    ...
    mfile := cat(grOptionMetricPath,`/`,metric,`.mpl`):
    #
...
for a to Ndim||gname do
    for b to Ndim||gname do
        for c to Ndim||gname do
           if diff ( grG_etabupbup_[gname,a,b], grG_xup_[gname,c] ) <> 0 then
               RETURN ( false ):
           fi:
        od:
    od:
