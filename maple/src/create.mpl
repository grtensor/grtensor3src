...
global  grG_ObjDef, grG_metricName, grF_contractFn:
local a,b,c,i, cousins,root, newList, baseObj, objIndices, dependSet,
  ...
  grG_ObjDef[object][grC_calcFnParms] := exprProd *
            grG_||root['grG_metricName','grG_operands',op(objIndices)];
  #
...
global grG_ObjDef, grG_operands:
 ...
 if newIndex = cdn or newIndex = cbdn or newIndex = pbdn then
   ...
   if newIndex = cbdn then
     ...
     grG_ObjDef[newObject][grC_calcFnParms] :=
        parse(convert(cat(`'`,`diff(grG_`,grG_ObjDef[rootObj][grC_root],
	      `[grG_metricName`,seq(`,a`||i||_,i=1..n-1),
	      `],grG_xup_[grG_metricName,`,
	      a||n||_,`])'` ),name)):
     grG_ObjDef[newObject][grC_depends] := { dependsOn }:
 fi:
 #
...
global grG_operands:
  ...
  #
  # od:
  # s := s + diff( grG_Xdndn_[a1_,a2_], grG_xup_[gname,a3_]
  #
  n := nops ( object ):
  baseSeq := seq ( a||i||_, i=1..n-1 ):
  if op ( n, object ) <> pbdn then
    if op ( n, object ) = cbdn then
      template[up] := grG_||root[grG_metricName,grG_operands,baseSeq] *
                    grG_rotbupbdnbdn_[grG_metricName,listIndex,s1,a||n||_]:
      template[dn] := -grG_||root[grG_metricName,grG_operands,baseSeq] *
                    grG_rotbupbdnbdn_[grG_metricName,s1,listIndex,a||n||_]:
    else
      template[up] := grG_||root[grG_metricName,grG_operands,baseSeq] *
                    grG_Chrdndnup_[ grG_metricName,s1, a||n||_, listIndex ]:

      template[dn] := -grG_||root[grG_metricName,grG_operands,baseSeq] *
                      grG_Chrdndnup_[ grG_metricName, listIndex, a||n||_, s1]:
    fi:
  ...
  if op ( n, object ) = cbdn or op ( n, object ) = pbdn then
    body := `&proc`( [dummy, iList], [s1,s], [], `&statseq` (
      ...
      `&for`( s1, 1, 1, Ndim[grG_metricName], true,
        `&statseq`(
        `&:=`(s, s + 'diff'( grG_||root[grG_metricName,grG_operands,baseSeq],
           grG_xup_[grG_metricName, s1] )*grG_ebdnup_[grG_metricName,a||n||_,s1] 
         ))
       ),
       loopStmt ) ):
  else
    body := `&proc`( [dummy, iList], [s1,s], [], `&statseq`(
            `&:=`(s, 'diff'( grG_||root[grG_metricName,grG_operands,baseSeq],
                                 grG_xup_[grG_metricName, a||n||_]) ),
             loopStmt) ):
  fi:
  body := procmake(body):
  grF_reassignMetrics():
