...
# saveEntry.
#-------------------------------------------------------------------
grF_saveEntry := proc ( G, metricdir )
local	metricfile, r, i, j:
	
	metricfile := convert ( cat ( metricdir, `/`, 
		G_gname, `.mpl` ), name):
	r := traperror ( writeto ( metricfile ) ):
	if r = lasterror then
		writeto ( terminal ):
		printf ( `Error: %a\n`, r ):
		RETURN():
	...
	for i to G_gdim do
		printf ( `%a := %a:\n`, x||i||_, G_coord[i] ):
	od:
	if assigned ( G_sig ) and not grOptionLLSC then
		printf ( `sig_ := %a:\n`, G_sig ):
	fi:
 	if assigned ( G_complex ) then
		printf ( `complex_ := %a:\n`, G_complex ):
	fi:
	if G_gtype = grG_g or G_gtype = grG_ds then
		for i to G_gdim do 
...
grF_usemetric := proc ( G_gdim, 
						...
						G_basisd,
						G_basisu)
global  Ndim, grG_xup_, grG_constraint, grG_Ndim, grG_metricName, grG_default_metricName, grG_Info_, grG_sig_, grG_complexSet_:
local 	i, btype:
	Ndim||(G_gname) := G_gdim:
	grG_metricName := G_gname:
	grG_default_metricName := grG_metricName:
	for i to G_gdim do
		grG_xup_[grG_metricName,i] := G_coord[i]:
	od:
...
local	a, b, maxSize, size:	
	...
	od:
	if maxSize < grG_maxTermLength then
		a := 'a': b := 'b':
...
local	a, b, maxSize, size, vname, v, e:
	...
	for a to gdim do
		...
		if size > maxSize then maxSize := size fi:
	od:
	if maxSize < grG_maxTermLength*gdim then
		v := array ( 1..gdim ):
		for a to gdim do
			for b to gdim do
				v[b] := basis[a,b]:
...
global	grG_etabupbup_, grG_etabdnbdn_:
local	ipu, ipd, a, b:
	...
	for a to ndim do 
		for b from a to ndim do
			...
			grG_etabdnbdn_[gname,b,a] := grG_etabdnbdn_[gname,a,b]:
			grG_etabupbup_[gname,a,b] := ipu[a,b]:
			grG_etabdnbdn_[gname,a,b] := ipd[a,b]:
		od:
	od:
...
global 	grG_ebdnup_, grG_lup_, grG_nup_, grG_mup_, grG_mbarup_:
local	vname, oname, a, b:
	for a to ndim do
		for b to ndim do
			grG_ebdnup_[gname,a,b] := basis[a,b]:
		od:
	od:
	...
	if ndim = 4 then
		...
		for a to 4 do
			for b to 4 do
				grG_||(vname[a])[gname,b] := basis[a,b]:
			od:
...
global 	grG_ebdndn_, grG_ldn_, grG_ndn_, grG_mdn_, grG_mbardn_,grG_ds_:
local	vname, oname, a, b:

	for a to ndim do
		for b to ndim do
			grG_ebdndn_[gname,a,b] := basis[a,b]:
		od:
	od:
...
global 	grG_gdndn_, grG_calc, grG_fnCode:
local	a, b:
	for a to ndim do
		for b from a to ndim do
			grG_gdndn_[gname,b,a] := grG_gdndn_[gname,a,b]:
			grG_gdndn_[gname,a,b] := metric[a,b]:
		od:
	od:
	...
	if Ndim[gname]=4 and grF_checkIfAssigned ( eta(bup,bup), test ) then
		...
		for a to 4 while nullt = true do
			for b to 4 while nullt = true do
				if grG_etabupbup_[gname,a,b] <> ip[a,b] then
					nullt := false:
				fi:
			od:
		od:
