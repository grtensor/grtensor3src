...
global grOptionCoordNames, grOptionWindows;
  ...
  rootStr := op(0,object):
   ...
   for a to nops(indices) do
	 ...
	 if grOptionCoordNames
         ...
         then
	   iList[a] := grG_xup_[grG_metricName, indices[a] ]:
	 else
...
global	grG_metricName, grF_ds_, displayList:
...
local	a, b, gname, Object, opList, displayZero:
	...
	if nops ( objectName ) = 1 then
		for a to Ndim||gname while displayZero = true do
			if grG_||Object[gname,opList,a]<>0 then
				displayZero := false:
			fi:
		od:
	else
		for a to Ndim||gname while displayZero = true do
			for b to Ndim||gname while displayZero = true do
				if grG_||Object[gname,opList,a,b]<>0 then
					displayZero := false:
				fi:
			od:
		od:
...
global grOptionTermSize;
	printf("PETER: termsize=%a", grOptionTermSize);
	gname := grG_metricName:
	Object := grG_ObjDef[objectName][grC_root]:
	...
	if nops ( objectName ) = 1 then
		...
		for a to Ndim||gname while rowsize < maxsize do
			rowsize := rowsize + length ( grG_||Object[gname,a] ):
		od:
	else
		rowsize := 0:
		for a to Ndim||gname while rowsize < maxsize do
			...
			for b to Ndim||gname while tmprowsize < maxsize do
				tmprowsize := tmprowsize + length ( grG_||Object[gname,a,b] ):
			od:
...
local	a, b, gname, Object, mat, opList, dispObj:
	...
	for a to Ndim||gname do
		for b to Ndim||gname do
			mat[a,b] := grG_||Object[opList,a,b]:
		od:
	od:
	dispObj := grF_symbTensorName ( objectName ):
