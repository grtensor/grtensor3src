  ...
  if newTensorDef = nodef then
	...
	grG_ObjDef[newObj][grC_depends] := {}:
     ...
     for b to nops(newTensorDef) do
        grG_||newRoot||newISeq||_[grG_default_metricName,b] := newTensorDef[b]:
     od:
...
#
symFn := `&:=`( `&catenate`(grG_,`&args`[2])[
  `&expseq`('grG_metricName','grG_operands',freeIndexSeq)], 
...
for a to nops ( idxList ) do
  ...
  if symIndexSeq <> freeIndexSeq then
    xrefs := `&:=`(
      `&catenate`(grG_,`&args`[2])[
      `&expseq`('grG_metricName','grG_operands', symIndexSeq)], 
      idxSign[a]*`&catenate`(grG_,`&args`[2])[
      `&expseq`('grG_metricName', 'grG_operands', freeIndexSeq)] ),
      xrefs:
    zeros := `&:=`(
      `&catenate`(grG_,`&args`[2])[
...
if asymSet <> {} then
  ...
  symFn := `&if`( zeroCond[asymNbr],
    `&statseq`(zeros,`&:=`(`&catenate`(grG_,`&args`[2])
    [`&expseq`('grG_metricName','grG_operands',freeIndexSeq)], 0 ) ),
    `&statseq`( xrefs, symFn ) ):
else
