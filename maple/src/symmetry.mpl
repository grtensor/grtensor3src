...
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
 ...
 global grG_profileList, grG_profileCount, grG_profileTimer, grG_profileSize:

 if grOptionVerbose and not grG_callComp then
    printf ("Component %s of %a\n", convert(iList,name), objectName):
 fi:
 if grG_simp then
      iSeq := gname,opSeq,op(iList):
      if grOptionProfile and grG_||root[iSeq] <> 0 then
	      grG_profileSize := grG_profileSize + length(grG_||root[iSeq]);
	      grG_profileCount := grG_profileCount + 1;
	      grG_profileList := [op(grG_profileList),length(grG_||root[iSeq])];
       fi:
       #
       # need simpArgs to allow NULLs to collapse before calling
       # single arg functions (otherwise get an error in e.g. eval)
       #
       simpArgs := grG_preSeq, grG_||root[iSeq], grG_postSeq:
       tic := time();
       grG_||root[iSeq] :=
 ...
 if grG_callComp then
       iSeq := gname,opSeq,op(iList):
       grF_component(objectName,iList, grG_||root[iSeq]);
...
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
	if grG_calc and assigned(calcFn) then
	   grG_||root[gname,opSeq] := calcFn(objectName,[]):
	fi:
        #
...
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
	...
	if grG_calc and assigned(calcFn)  then
	   ...
	   if a1_ <> a2_ then
	      grG_||root[gname,opSeq,a2_,a1_] := grG_||root[gname,opSeq,a1_,a2_]:
	   fi:
	   # assignement
	   grG_||root[gname,opSeq,a1_,a2_] := calcFn(objectName,[a1_,a2_]):
	fi:
	...
	if grG_calc and assigned(calcFn)  then
	   ...
	   if a1_ <> a2_ then
	      ...
	      grG_||root[gname,opSeq,a1_,a2_] := calcFn(objectName,[a1_,a2_]):
              ...
              grG_||root[gname,opSeq,a1_,a2_] := 0:
           fi:
	fi:
	grF_symCore(objectName, [a1_,a2_], root):
      od;
  ...
  # three index objects, sym w.r.t first two
	...
	for a3_ to Ndim[grG_metricName] do
	  if grG_calc and assigned(calcFn)  then
	     # assignments here reflect symmetry
	     if a1_ <> a2_ then
		grG_||root[gname,opSeq,a2_,a1_,a3_] :=  grG_||root[gname,opSeq,a1_,a2_,a3_]:
	     fi:
	     # assignment
	     grG_||root[gname,opSeq,a1_,a2_,a3_] := calcFn(objectName,[a1_,a2_,a3_]):
	  fi:
...
grG_symmetry[grF_sym_3a12] := {[[2,1,3],-1]}:
	  ...
	  if grG_calc and assigned(calcFn)  then
		...
		grG_||root[gname,opSeq,a2_,a1_,a3_] :=  -grG_||root[gname,opSeq,a1_,a2_,a3_]:
	        ...
	        grG_||root[gname,opSeq,a1_,a2_,a3_] := calcFn(objectName,[a1_,a2_,a3_]):
                ...
                grG_||root[gname,opSeq,a1_,a2_,a3_] := 0:
             fi:
	  fi:
	  grF_symCore(objectName, [a1_,a2_,a3_], root):
	od;
  ...
  # three index objects, sym w.r.t first two
	...
	for a2_ to Ndim||grG_metricName do
	  if grG_calc and assigned(calcFn)  then
	     # assignments here reflect symmetry
	     if a1_ <> a3_ then
		grG_||root[gname,opSeq,a3_,a2_,a1_] := -grG_||root[gname,opSeq,a1_,a2_,a3_]:
 	        # assignment
	        grG_||root[gname,opSeq,a1_,a2_,a3_] := calcFn(objectName,[a1_,a2_,a3_]):
             else
                grG_||root[gname,opSeq,a1_,a2_,a3_] := 0:
  ...
  # three index objects, sym w.r.t first two
	...
	for a1_ to Ndim||grG_metricName do
	  if grG_calc and assigned(calcFn)  then
	     # assignments here reflect symmetry
	     if a3_ <> a2_ then
		grG_||root[gname,opSeq,a1_,a3_,a2_] := -grG_||root[gname,opSeq,a1_,a2_,a3_]:
	        # assignment
	        grG_||root[gname,opSeq,a1_,a2_,a3_] := calcFn(objectName,[a1_,a2_,a3_]):
	     else
                grG_||root[gname,opSeq,a1_,a2_,a3_] := 0:
	...
	for a3_ to Ndim[grG_metricName] do
	  for a4_ from a3_ to Ndim[grG_metricName] do
	    if grG_calc and assigned(calcFn)  then
	      # assignments here reflect symmetry
	      if a1_ <> a2_ then
		 grG_||root[gname,opSeq,a2_,a1_,a3_,a4_] := grG_||root[gname,opSeq,a1_,a2_,a3_,a4_]:
		 if a3_ <> a4_ then
		    grG_||root[gname,opSeq,a2_,a1_,a4_,a3_] := grG_||root[gname,opSeq,a1_,a2_,a3_,a4_]:
		 fi:
	      fi:
	      if a3_ <> a4_ then
		 grG_||root[gname,opSeq,a1_,a2_,a4_,a3_] := grG_||root[gname,opSeq,a1_,a2_,a3_,a4_]:
	      fi:
	      # assignment
	      grG_||root[gname,opSeq,a1_,a2_,a3_,a4_] := calcFn(objectName,[a1_,a2_,a3_,a4_]):
	    fi:
...
local x:
	...
	for a2_ to Ndim[grG_metricName] do
	  for a3_ to Ndim[grG_metricName] do
	       grG_||root[gname,opSeq,a1_,a1_,a2_,a3_] := 0;
	       grG_||root[gname,opSeq,a1_,a2_,a3_,a3_] := 0;
	  od;
	od;
      od;
    fi:
	...
	for a3_ from a1_ to Ndim[grG_metricName]-1 do
	  if a3_ = a1_ then x := a2_ else x := a3_+1 fi;
	    for a4_ from x to Ndim[grG_metricName] do
		 ...
		 grG_||root[gname,opSeq,a3_,a4_,a1_,a2_] := grG_||root[gname,opSeq,a1_,a2_,a3_,a4_]:
		 grG_||root[gname,opSeq,a3_,a4_,a2_,a1_] := -grG_||root[gname,opSeq,a1_,a2_,a3_,a4_]:
		 grG_||root[gname,opSeq,a4_,a3_,a1_,a2_] := -grG_||root[gname,opSeq,a1_,a2_,a3_,a4_]:
		 grG_||root[gname,opSeq,a4_,a3_,a2_,a1_] := grG_||root[gname,opSeq,a1_,a2_,a3_,a4_]:
		 #
	...
	for a3_ to Ndim[grG_metricName]-1 do
	  for a4_ from a3_+1 to Ndim[grG_metricName] do
	    if grG_calc and assigned(calcFn)  then
	      # zero by symmetry
	      grG_||root[gname,opSeq,a1_,a2_,a3_,a3_] := 0:
	      grG_||root[gname,opSeq,a1_,a2_,a3_+1,a3_+1] := 0:
...
local x:
	...
	for a2_ to Ndim[grG_metricName] do
	  for a3_ to Ndim[grG_metricName] do
	       grG_||root[gname,opSeq,a1_,a1_,a2_,a3_,a5_] := 0;
	       grG_||root[gname,opSeq,a1_,a2_,a3_,a3_,a5_] := 0;
	  od;
	od;
      od;
    od:
	...
	for a3_ from a1_ to Ndim[grG_metricName]-1 do
	  if a3_ = a1_ then x := a2_ else x := a3_+1 fi;
		...
		if grG_calc and assigned(calcFn)  then
		  # assignments here reflect symmetries
		  grG_||root[gname,opSeq,a2_,a1_,a3_,a4_,a5_] := -grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_]:
		  grG_||root[gname,opSeq,a1_,a2_,a4_,a3_,a5_] := -grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_]:
		  grG_||root[gname,opSeq,a2_,a1_,a4_,a3_,a5_] := grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_]:
		  # now with first & second pair exchanged
		  grG_||root[gname,opSeq,a3_,a4_,a1_,a2_,a5_] := grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_]:
		  grG_||root[gname,opSeq,a3_,a4_,a2_,a1_,a5_] := -grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_]:
		  grG_||root[gname,opSeq,a4_,a3_,a1_,a2_,a5_] := -grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_]:
		  grG_||root[gname,opSeq,a4_,a3_,a2_,a1_,a5_] := grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_]:
		  #
		  # put assigment AFTER sym ,so when simplification occurs
		  # the symmetrized objects don't need reassigment
		  #
		  grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_] := calcFn(objectName,[a1_,a2_,a3_,a4_,a5_]):
	       fi:
...
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
	...
	for a3_ to Ndim[grG_metricName] do
	  for a4_ to Ndim[grG_metricName] do
	    for a5_ to Ndim[grG_metricName] do
	      if grG_calc and assigned(calcFn)  then
	        grG_||root[gname,opSeq,a1_,a2_,a3_,a4_,a5_] :=
