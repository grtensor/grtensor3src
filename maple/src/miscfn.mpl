...
#----------------------------------------------------------
# definition of standard delta fuction
#----------------------------------------------------------
...
#local new_a, new_b;
   ...
   #
   # need to ensure that a arguments like
   #  kdelta( `x{^a}`, r)
   #  kdelta( grG_xup_[gname, a1_], r)
   #
   # don't get collapsed to 1 or 0 to early.
   #
   # The former is a bit of a pain since Maple
   # thinks that `x{^a}` is a name and all
   # names are strings.
   #
   #
#   if type( a, {indexed,function,procedure}) or 
#      type(b, {indexed,function,procedure}) or
#      traperror( parse(a)) = lasterror or
#      traperror( parse(b)) = lasterror then
#     RETURN('kdelta'(a,b) );
...
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
global  grG_rootSet, grG_usedNameSet:
...
#----------------------------------------------------------
# grF_permSign
# - shufle indices in permList by interchange of adjacent
