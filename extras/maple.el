...
(defvar maple-indent 2 "*This variable gives the indentation in Maple-Mode")
...
(defun maple-newline ()
  ...
  (let  ((hpos (current-indentation)))
    (if (< (current-column) (current-indentation))
      (newline) (newline) (indent-to hpos))))
...
;; Hilit patterns for use with hilit19 (by Denis):

(hilit-set-mode-patterns
 'maple-mode
 '(("[#].*$" nil comment)
   ("`[^`\n]*`" nil string)
   ("[ \t\n]\\(for\\|while\\|do\\|in\\|from\\|to\\|by\\|and\\|or\\|od\\|if\\|elif\\|then\\|else\\|fi\\)[ :;\t\n]" nil keyword)
   ("[ \t:=0-9A-Aa-z_]*\\(proc[ \t\n0-9A-Za-z():;,_]*)[\n]+\\|end[ :;\n]\\)" nil defun)
   ("\\(local\\|global\\)" nil type)
   ("\\(RETURN\\|ERROR\\)" nil keyword)
   )
