...
option `Copyright 1994 by Peter Musgrave, Denis Pollney and Kayll Lake`;
global  grF_DIFF, grF_INT, grG_newObjects, grG_ObjDef:
local a:
	grF_DIFF := 'grF_DIFF':
	grF_INT := 'grF_INT':
	grF_unassignForDef();
  ...
  for a to nops(grG_savedObjects) do
    ...
    if assigned( grG_ObjDef[object][grC_root]) then
     ...
     if grG_ObjDef[object][grC_indexList] <> [] then
      ...
      #
      # PM2016 - This is broken - will need another way
...
local a, b, argList, argSeq, fileName, object, objName,
 ...
 #
 if not type( args[nargs], string) then
   ERROR(`The last argument must be a string (the filename)`);
 fi:
 if nargs < 2 then
 ...
 for a to nops(argList) do
   if assigned (grG_ObjDef[argList[a]][grC_grdefArgs]) then
     ...
     for b in entries do
       ...
       if entry <> grC_calcFn and entry <> grC_symmetry then
          if not type ( grG_ObjDef[argList[a]][entry], name ) then
            fprintf (fd, `grG_ObjDef[%a][%a] := %a:\n`, argList[a], entry,
              grG_ObjDef[argList[a]][entry] ):
          else
            fprintf (fd, `grG_ObjDef[%a][%a] := ``%s``:\n`, argList[a], entry,
              grG_ObjDef[argList[a]][entry] ):
          fi:
       fi:
     od:
     ...
     if member ( grG_ObjDef[argList[a]][grC_calcFn], grG_calcFnSet ) then
       ...
       fprint (fd,  grG_ObjDef[argList[a]][grC_calcFn] ):
     fi:
     fprintf (fd, `:\n` ):
     ...
     if not member ( grG_ObjDef[argList[a]][grC_symmetry], symFnSet ) then
       ...
       fprintf (fd, "%a", grG_ObjDef[argList[a]][grC_symmetry] ):
       fprintf(fd,`:\n`):
     fi:
     fprintf ( fd, `\n` ):
   fi:  
